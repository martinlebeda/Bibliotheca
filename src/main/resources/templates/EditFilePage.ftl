...
<div class="edit-form">
    ...
    <#if (cover?has_content) >
        ...
        <fieldset class="editFormSection">
            <p><label for="dbknih">DatabazeKnih.cz:</label>
            ...
            <p><label for="bookdescription">Description:</label>
                <textarea id="bookdescription" name="bookdescription" cols="50" rows="5"
                          style="width: 100%"><#if (desc?has_content) >${desc}</#if></textarea>
            </p>
        </fieldset>

        <div style=" text-align: right; padding: 20px">
            <a class="file-external_search" href="https://www.google.cz/search?q=${name}" target="_blank">Google</a>
            <a class="file-external_search"

            <#if (dbknih?has_content)>
               href="${dbknih}"
            <#else>
               href="http://www.databazeknih.cz/search?q=${bookname}&hledat=&stranka=search"
            </#if>

               target="_blank">DatabázeKnih</a>
            <input type="submit" name="tryDbKnih" value="try DatabázeKnih"></input>
